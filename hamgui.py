...
def setup_window() -> sg.Window:
    ...
    new_window = sg.Window("HamPi",[columns,[pusher],[exit_button]],no_titlebar=True,keep_on_top=True,size=(WIDTH,HEIGHT)) # TODO: Make screen size auto-detect
    return new_window
...
def handle_window_event(event, values):
    ...
    for component in active_components:
        if event == component.ui_key:
            component.ui_control_interacted(values.get(event,None))
    return True
